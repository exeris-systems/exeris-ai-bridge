...
6. **Cross-repo obligations.** Each is a companion ask in the owning repo, tracked in the ROADMAP's cross-repo table: **`exeris-platform`** — artifact-launchable LSP and `exeris/previewMutation`; **`exeris-kernel`** — artifact-launchable diagnostics CLI; **`exeris-sdk`** — `annotation-catalog.json` plus a published AST JSON Schema, generated by the reflection mechanism `AnnotationContractTest` already uses; **`exeris-tooling`** — stable diagnostic IDs in `ExerisDomainProcessor`, which emits free text today.

   > **Editorial note (2026-09-02).** This item originally pinned a milestone number to each ask. The numbers are removed rather than corrected. This document outranks `ROADMAP.md`, `README.md` and `CLAUDE.md` under the repo's documentation-precedence rule, so scheduling data held here makes every reordering an amendment to the founding decision — and when the milestones were reordered on 2026-09-02 (`build:*`/`caps:*` to 0.6.0, `sdk:*` to 0.7.0, after the `sdk:*` content source was answered upstream), the highest-precedence source was the one left contradicting everything below it. The obligations themselves are unchanged; **which** milestone consumes each is the ROADMAP's to say, and this item already delegated the tracking there.
...
### Why it is needed now rather than at the milestone it was then scheduled for
