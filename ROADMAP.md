...
- [x] **LSP server discovery** — `EXERIS_LSP_COMMAND` env var (default: `mvn -f <ecosystemRoot>/exeris-platform/exeris-platform-lsp/pom.xml exec:java`); lazy spawn on first `lsp:*` call; cached handle
