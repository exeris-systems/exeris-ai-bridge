...
>
> **Gate (verified 2026-08-27): this milestone has no content source yet.** `exeris-sdk` v0.11.0 is released, and none of its artifacts carry a machine-readable catalog — the jars hold classes and Maven metadata only, and the GitHub release has no assets. The annotations are `@Retention(SOURCE)`, so nothing recovers the per-attribute purpose text from a class file either. Every tool below reads a catalog that does not exist yet, so 0.6.0 is **blocked on the upstream ask**, not on work in this repo. Starting it here would mean hand-writing the catalog — which the `sdk:list_annotations` note explains is stale one release later — or parsing the sources jar in TypeScript, which reimplements the SDK's own semantics on the wrong side of the boundary.
...
| 0.6.0     | `exeris-sdk`               | **`annotation-catalog.json` as a published artifact** — annotation names, targets, retention, attributes with types/defaults, deprecation status + replacement. Generated by an SDK-side build step, not hand-maintained here. `AnnotationContractTest` already discovers every annotation by classpath reflection, so the emitter is that mechanism plus a serializer. Also: a published JSON Schema for `DomainMetadata` and the AST records. **Still open after v0.11.0 (verified 2026-08-27):** every 0.11.0 jar ships classes and Maven metadata only — no JSON resource in `annotations`, `source-model`, `source-model-io` or `composition-spec` — and the GitHub release carries no assets, so there is nothing for `prepack` to vendor. Deriving it bridge-side is not a fallback: the annotations are `@Retention(SOURCE)`, and the one thing a class file cannot carry is the javadoc that supplies each attribute's purpose. **This is the whole of 0.6.0's content** — the family has no other source |
...
Every ask is small and well-shaped; none requires a new subsystem. After the 2026-08-27 reconciliation the critical path is **0.6.0 (annotation catalog)** — the content the whole `sdk:*` family serves, and **still the one ask with no upstream progress**, now against a v0.11.0 that shipped and left it exactly where it was. Behind it sits the **platform half of 0.5.0 (launchable LSP artifact)**, without which `lsp:*` stays contributor-only. The kernel half of 0.5.0 is done upstream and merely unreleased, and `caps:*` (0.7.0) turned out to have a shipped producer rather than a 2027 one.

**Upstream releases verified 2026-08-27** — `exeris-kernel` v0.11.0 (dev `0.12.0-SNAPSHOT`), `exeris-sdk` **v0.11.0** (`main` sits on the release version; no next dev line opened yet), `exeris-tooling` v0.7.0 (dev `0.8.0-SNAPSHOT`), `exeris-platform` **no releases at all**. Re-verify the versions whenever a row is edited, because every ask below is stated against a moving upstream — and check what a release *contains*, not only that it exists: v0.11.0 landed without moving the 0.6.0 ask an inch.
