...
**0.6.0 — the application developer's own project: what the build emitted, what it will overwrite, and what its errors mean.** ADR-025 ACCEPTED (2026-05-15), five amendments since. All six tool families are implemented and verified end-to-end: `docs:*` (9 tools, filesystem-bound against `exeris-docs`), `lsp:*` (3 tools, bound to the read-only `exeris/*` slice in `exeris-platform-lsp`), `kernel:*` (4 tools, one per `KernelDiagnostics` SPI method, over NDJSON — read-only and cap-blind by construction, so no `kernel-list_capabilities`), `build:*` (5 tools, reading the `DomainMetadata` the developer's own build emitted, the codegen output tree beside it, and the diagnostics the build printed), `caps:*` (2 tools, reading the build-time `cap-manifest.json` of the same project), and `bridge:*` (2 tools, the server's own diagnostic surface). Both child families keep an opt-in live integration test (`EXERIS_LSP_IT=1`, `EXERIS_KERNEL_IT=1`).

What 0.6.0 adds is the P2 half of the surface, and one discipline runs through all of it: **the bridge reads what the pipeline recorded, and does not recompute it.** `caps:*` reads the emitted `cap-manifest.json` rather than re-resolving the `@Requires`→`@Provides` DAG. `build-get_domain_metadata` validates the envelope and passes the AST through verbatim rather than re-declaring 40+ record components in TypeScript. `build-explain_artefacts` reads the codegen output manifest rather than predicting emission — only 2 of the 13 generators state their condition where a mirror could read it, and the other eleven decide by a null sentinel inside `generate(...)`. Each of those would have been a second implementation of a contract another repo owns, wrong the first time it moved. The one place with no recorded answer to read is `build-explain_diagnostic`, because the build emits no diagnostic codes — so it matches on message text, anchors on the SDK's own public vocabulary rather than on prose, keeps a table of real upstream messages that fails the build when an anchor stops finding its own message, and reports a miss as a gap in its catalogue rather than as a verdict on the diagnostic.

What 0.5.0 added is not tools but the ability to run without the ecosystem: `loadConfig()` never throws, a dependency that does not resolve takes its own family dark with a `reason` and a `remedy`, `tools/list` stays invariant so the 1.0 freeze remains implementable, each child is resolved through a launch ladder (`EXERIS_*_COMMAND` → `EXERIS_*_JAR` → local Maven repository → source tree) whose rung order follows the persona, and a read-only reference corpus ships inside the package. A CI job (`scripts/p2-smoke.mjs`) packs the tarball, installs it into a scratch directory holding only an application project, and speaks MCP to it with an empty `HOME` and no `EXERIS_*` set — the P2 claim is tested, not asserted.

Two upstream dependencies remain open and are documented rather than papered over: `exeris-platform-lsp` publishes no runnable jar, so `lsp:*` has no local-repository rung and stays contributor-only; and the published `exeris-kernel-diagnostics-cli:0.11.0` jar does not boot — **fixed upstream on the 0.12 development line, so the remedy arrives with kernel 0.12.0 rather than with a change here** (see the note under [Wiring into an agent](#wiring-into-an-agent)). Next: `sdk:*` (0.7.0) — the annotation catalog that fills the bundle, answered upstream on the `exeris-sdk` 0.12.0 line and waiting on that release. MCP resources + prompts are at 0.9.0. **0.6.0 is not published to npm.** Publication waits on kernel 0.12.0, sdk 0.12.0 and tooling 0.9.0 reaching Maven Central. What makes that gate matter: until they do, every `eu.exeris:*` artifact the launch ladder resolves lives in GitHub Packages, which needs a `PACKAGES_READ_TOKEN` — so a first npm release would reach exactly the P2 developers who cannot resolve what it points at.
...
`tools/list` advertises all 25 tool definitions (9 `docs:*`, 3 `lsp:*`, 4 `kernel:*`, 5 `build:*`, 2 `caps:*`, 2 `bridge:*` — all live); `tools/call` on `docs-get_adr` returns the ADR-024 body.
...
src/
  ...
  tools/
    ...
    build/index.ts           build-list_domains, build-get_domain_metadata,
                             build-explain_artefacts, build-get_detach_state,
                             build-explain_diagnostic — reads the
    ...
    build/diagnostics.ts     catalogue of the build's own diagnostics, matched on message text
                             because the build emits no codes; a miss is a catalogue gap
