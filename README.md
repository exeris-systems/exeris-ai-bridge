...
`EXERIS_LSP_COMMAND` (optional) is how the `lsp:*` family launches its `exeris-platform-lsp` child — a whitespace-separated command + args (no shell quoting; the process is exec'd directly). It defaults to `mvn -f <ecosystemRoot>/exeris-platform/exeris-platform-lsp/pom.xml exec:java`. The child is spawned lazily on the first `lsp:*` call and cached. Until the companion LSP custom requests land, `lsp:*` calls return a structured "not yet supported" result rather than failing.
